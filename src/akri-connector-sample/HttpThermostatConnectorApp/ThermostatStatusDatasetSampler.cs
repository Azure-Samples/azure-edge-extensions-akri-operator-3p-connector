...
namespace HttpThermostatConnectorAppProjectTemplate
{
    internal class ThermostatStatusDatasetSampler : IDatasetSampler
    {
        ...
        public async Task<byte[]> SampleAsync(Dataset dataset, AssetEndpointProfileCredentials? assetEndpointProfileCredentials = null, CancellationToken cancellationToken = default)
        {
            try
            {
                string? httpServerRequestPath = null;

                if (dataset.DataPointsDictionary!.ContainsKey("desired_temperature"))
                {
                    ...
                    httpServerRequestPath = httpServerDesiredTemperatureDataPoint.DataSource!;
                }
                
                if (dataset.DataPointsDictionary!.ContainsKey("actual_temperature"))
                {
                    ...
                    // In this sample, both the datapoints have the same datasource, so only one HTTP request is needed.
                    httpServerRequestPath = httpServerActualTemperatureDataPoint.DataSource!;
                }

                if (httpServerRequestPath == null)
                {
                    throw new InvalidOperationException("No data point found for the dataset");
                }
                ...
                // In this sample, both the datapoints have the same datasource, so only one HTTP request is needed.
                var response = await _httpClient.GetAsync(httpServerRequestPath);
