...
namespace TcpConnector
{
	public class TcpConnectorAppWorker : BackgroundService
	{
		...
		private readonly ILogger<TcpConnectorAppWorker> _logger;
		...
		public TcpConnectorAppWorker(ILogger<TcpConnectorAppWorker> logger,
			MqttSessionClient mqttSessionClient, IDatasetSamplerFactory datasetSamplerFactory)
		{
			_logger = logger;
		...
		protected override async Task ExecuteAsync(CancellationToken cancellationToken)
		{
			try
			{
				...
				mqttConnectionSettings.TcpPort = mqttConnectionSettings.UseTls ? 18883 : 1883;
				...
				_adrClient.AssetChanged += (sender, args) =>
				{
					if (args.ChangeType == ChangeType.Deleted)
					{
						_logger.LogInformation(
							$"Recieved a notification the asset with name {args.AssetName} has been deleted.");
					...
					{
						_logger.LogInformation(
							$"Recieved a notification an asset with name {args.AssetName} has been created.");
					...
					{
						_logger.LogInformation(
							$"Recieved a notification the asset with name {args.AssetName} has been updated.");
		...
		private async Task StartSamplingAssetAsync(string assetName, CancellationToken cancellationToken = default)
		{
			...
			else
			{
				foreach (string datasetName in _assets[assetName].DatasetsDictionary!.Keys)
				{
					...
					TimeSpan defaultSamplingInterval = TimeSpan.FromMilliseconds(
						_assets[assetName].DefaultDatasetsConfiguration!.RootElement.GetProperty("samplingInterval")
							.GetInt16());

					TimeSpan samplingInterval = defaultSamplingInterval;
					if (dataset.DatasetConfiguration != null
					    && dataset.DatasetConfiguration.RootElement.TryGetProperty("samplingInterval",
						    out JsonElement datasetSpecificSamplingInterval))
					{
						samplingInterval = TimeSpan.FromMilliseconds(datasetSpecificSamplingInterval.GetInt16());
					}

					_logger.LogInformation(
						$"Will sample dataset with name {datasetName} on asset with name {assetName} at a rate of once per {(int)samplingInterval.TotalMilliseconds} milliseconds");
					Timer datasetSamplingTimer = new(SampleDataset, new DatasetSamplerContext(assetName, datasetName),
						0, (int)samplingInterval.TotalMilliseconds);
					_samplers[assetName][datasetName] = datasetSamplingTimer;

					string mqttMessageSchema = dataset.GetMqttMessageSchema();
					_logger.LogInformation(
						$"Derived the schema for dataset with name {datasetName} in asset with name {assetName}:");
					...
					if (doSchemaWork)
					{
						...
						if (schema == null)
						{
							throw new InvalidOperationException(
								"Failed to register the message schema with the schema registry service");
						}
		...
		private async void SampleDataset(object? status)
		{
			...
			{
				_logger.LogInformation(
					$"Dataset with name {datasetName} in asset with name {samplerContext.AssetName} was deleted. This sample won't sample this dataset anymore.");
				return;
			}
			...
			{
				_datasetSamplers.TryAdd(datasetName,
					_datasetSamplerFactory.CreateDatasetSampler(_assetEndpointProfile!, asset, dataset));
			}
			...
			datasetSampler.StartSampling(dataset, (sender, args) => serializedPayload = args.Buffer);
			datasetSampler.StopSampling(); // will be restarted by timer based on sampling interval
			
			_logger.LogInformation(
				$"Read dataset with name {dataset.Name} from asset with name {assetName}. Now publishing it to MQTT broker: {Encoding.UTF8.GetString(serializedPayload)}");
			...
			if (puback.ReasonCode != MqttClientPublishReasonCode.Success
			    ...
			    MqttClientPublishReasonCode
				    .NoMatchingSubscribers) // There is no consumer of these messages yet, so ignore this expected NoMatchingSubscribers error
			{
				_logger.LogInformation(
					$"Received unsuccessful PUBACK from MQTT broker: {puback.ReasonCode} with reason {puback.ReasonString}");
			}
		}
	}
}